...
function propertyChangedHandler(
  ...
  signalObjPath: string
) {
  const valueVariant = changedProperties.lookup_value('Value', null)
  if (!valueVariant) return
  const bytes = valueVariant.get_data_as_bytes().unref_to_data()
  if (!bytes || bytes.length === 0) return
  return bytes[0]
}

function subscribeToBatUpdates(
  bus: Gio.DBusConnection,
  characteristics: { [path: string]: Gio.DBusProxy }
) {
  console.log('updated')
...
function enableNotifications(characteristics: {
  [path: string]: Gio.DBusProxy
}) {
  for (const [path, proxy] of Object.entries(characteristics)) {
    if (
      !proxy
        .get_cached_property('Flags')
        .deepUnpack<string[]>()
        .includes('notify')
    )
      continue
