...
export const TabsCarousel = () => {
  ...
  tabs.subscribe(tabs => {
    ...
    tabs.forEach(tab => {
      const tabView = Tab(tab, () => { carousel.scroll_to(tabView, true) })
  ...
  selectedTab.subscribe(tab => {
    for (let i = 0; i < carousel.get_n_pages(); i++) {
      if (carousel.get_nth_page(i))
    }
    
    const scrollTo = viewmap.get(tab.id)
    if (scrollTo) {
      carousel.scroll_to(scrollTo, true)
    }
  })
...
const Tab = (tab: Tab, onClick: () => void) => <ToggleButton
  label={tab.title}
  setup={self => self.connect("toggled", () => {
    if (self.get_active()) {
      onClick()
    }
  })}
