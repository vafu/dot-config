...
export class Label extends astalify(Gtk.Label) {
  static {
    GObject.registerClass({ GTypeName: 'Label' }, this)
  }
...
export class Button extends astalify(
  ...
  static {
    GObject.registerClass({ GTypeName: 'Button' }, this)
  }
  constructor(props?: ButtonProps, ...children: Array<BindableChild>) {
...
>
export class Popover extends astalify(
  Gtk.Popover,
  Gtk.Popover.name,
  (widgets, self) => {
    self.set_child(<Box>{widgets}</Box>)
  ...
  static {
    GObject.registerClass({ GTypeName: 'Popover' }, this)
  }
  constructor(props?: PopoverProps, ...children: Array<BindableChild>) {
    super({ children, ...props } as any)
  }
}

export type IconProps = ConstructProps<Icon, Gtk.Image.ConstructorProps>
export class Icon extends astalify(Gtk.Image, Gtk.Image.name) {
  static {
...
export class Box extends astalify(Gtk.Box) {
  static {
    GObject.registerClass({ GTypeName: 'Box' }, this)
  }
  constructor(props?: BoxProps, ...children: Array<BindableChild>) {
    super({ children, ...props } as any)
  }
}
...
export type CenterBoxProps = ConstructProps<
  CenterBox,
  Gtk.CenterBox.ConstructorProps
>
export class CenterBox extends astalify(
  ...
  (widgets, self) => {
    if (widgets.length > 3) {
      throw Error(
        'Cannot set > 3 widgets to centerbox, consider wrapping into more Boxes'
      )
    }
    if (widgets[0]) {
  ...
  static {
    GObject.registerClass({ GTypeName: 'CenterBox' }, this)
  }
  constructor(props?: CenterBoxProps, ...children: Array<BindableChild>) {
    super({ children, ...props } as any)
  }
}
